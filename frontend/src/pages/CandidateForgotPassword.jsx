...
import { Mail, Lock, Key, ArrowLeft, Star, Sparkles, Shield } from 'lucide-react';
...
const CandidateForgotPassword = () => {
    ...
    const handleRequestOTP = async (e) => {
        ...
        setLoading(true);
        try {
            const { data } = await axios.post(`${backendUrl}/user/forgot-password`, { email });
            if (data.success) {
    ...
    const handleVerifyOTP = async (e) => {
        ...
        setLoading(true);
        try {
            const { data } = await axios.post(`${backendUrl}/user/verify-reset-otp`, { email, otp });
            if (data.success) {
                toast.success(data.message);
                setStep(3);
            } else {
            ...
            <div className="w-full max-w-md">
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                    ...
                    <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-8 text-center">
                        ...
                        <p className="text-blue-100 text-sm mt-2">Candidate Account</p>
                    </div>
                        ...
                        <div className="flex items-center justify-between mb-8">
                            {[1, 2, 3].map((s) => (
                                <React.Fragment key={s}>
                                    ...
                                    {s < 3 && (
                                        <div
                                            className={`flex-1 h-1 mx-2 rounded transition-all ${step > s ? 'bg-gradient-to-r from-blue-600 to-purple-600' : 'bg-gray-200'
                                                }`}
                                        />
                                    )}
                                </React.Fragment>
                            ...
                            <form onSubmit={handleVerifyOTP} className="space-y-6">
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            placeholder="000000"
                                            className="w-full pl-11 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-center text-2xl font-mono tracking-widest"
                                            maxLength={6}
                                            required
                                        />
                                    </div>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 rounded-lg font-semibold hover:from-blue-700 hover:to-purple-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
                                    >
                    ...
                    <div className="bg-gray-50 px-8 py-4 border-t border-gray-100">
                        <p className="text-center text-sm text-gray-600">
                            ...
                            <Link to="/candidate-login" className="text-blue-600 font-medium hover:text-blue-700">
                                Sign in
                            </Link>
                        </p>
                    </div>
